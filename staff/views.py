...
from .serializers import AppointmentSerializer, StaffNoteSerializer, AuditTrailSerializer
from accounts.permissions import IsAnyStaff, IsStaffOrAdmin
...
class ReferAppointmentView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not referred_to_id:
            return Response({'message': 'referred_to field is required'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            referred_to_user = User.objects.get(id=referred_to_id)
        ...
        appointment.status = role_status_map.get(referred_to_user.role, 'REFERRED')
        appointment.save()
...
class StaffNoteViewSet(SuccessMessageViewSetMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsAnyStaff]
    create_success_message = 'Note added successfully'
...
class AuditTrailListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user = self.request.user
        if user.role == 'PATIENT':
            return AuditTrail.objects.filter(patient=user)
...
class StaffPatientListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return User.objects.filter(role='PATIENT').order_by('username')
